...
bool is_allowed(int y, int x, cell_t **player, int board_len){
    int neighbors=0;
    //niedozwoleni
    if (check_neighbor(-1, -1, y, x, player, board_len) || check_neighbor(1, 1, y, x, player, board_len) || check_neighbor(-1, 1, y, x, player, board_len) || check_neighbor(1, -1, y, x, player, board_len)){
        return false;
    }
    int x_last_neigh, y_last_neigh;
    int allowed_neighboars[4][2] = {{-1,0}, {0,-1}, {1, 0},{0,1}};
    for(int i=0; i < 4; i++){
        if (check_neighbor(allowed_neighboars[i][0], allowed_neighboars[i][1], y, x, player, board_len)){
            neighbors += 1;
    ...
    if (neighbors > 1){
        ...
        int ship_len = 1;
        return check_valid_ship(y + y_last_neigh, x + x_last_neigh, y_last_neigh, x_last_neigh, &ship_len, player, board_len);
...
void test_is_alowed(){
    ...
    c.y = 1;
    {
        ...
        c.x = 0;
        c.y = 0;
        assert(is_allowed(0, 0, max_len_bf, size) == false);
        free_battlefield(max_len_bf, size);
    }
    {
        int size = 2;
        BATTLEFIELD(simple, ARR({{SHIP,EMPTY}, {EMPTY,EMPTY}}), 2)
        assert(is_allowed(0, 0, simple_bf, size) == true);
        c.x=1;
        c.y=0;
        assert(is_allowed(0, 1, simple_bf, size) == true);
        simple_bf[0][1] |= SHIP;
        assert(is_allowed(0, 1, simple_bf, size) == false);
